...
int main() {
	...
	int K_val = N;
	FILE * f1 = fopen("przyblizenia.dat", "w");
	FILE * f2 = fopen("macierzX.dat", "w");
	FILE * f3 = fopen("macierzD.dat", "w");
	FILE * f4= fopen("przyblizenia.dat", "w");
	...
	for (int i = 0; i < N; i++) {
        ...
        fprintf(f4, "%2d ", i + 1);
        for (int j = 0; j < N; j++) 
            fprintf(f4, "%15g ", przyblizenia[i][j]);
        fprintf(f4, "\n");
    }
...
void orthogonalization(double * v1, double * v2, int size) {
	double tmp = vectorVector(v1, v2, size);
