...
set style line 2 lc rgb '#377EB8' pt 1 ps 1 lt 1 lw 2 # blue
...
set out 'drugiePochodne.png'
set xlabel 'x'
set ylabel "f_1''(x)"
set xrange [-5:5]
set grid
plot 8*x*x/((1+x*x)*(1+x*x)*(1+x*x)) - 2/((1+x*x)*(1+x*x)) w l ls 3 t "f_1''(x) analitycznie", \
'results1.dat' u 1:2 w p ls 2 t 'm_1', \
'' u 1:3 w p ls 1 t "f_1''(x) numerycznie"